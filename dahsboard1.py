...
def capture_frames():
    ...
    while cap.isOpened():
        ...
        if not ret:
            ...
            break
        # Konversi frame ke format RGB (karena OpenCV menggunakan BGR)
        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        yield frame
    cap.release()

st.title("Streaming Kamera dengan OpenCV")

# Placeholder untuk menampilkan frame
placeholder = st.empty()

# Menampilkan frame secara terus-menerus
